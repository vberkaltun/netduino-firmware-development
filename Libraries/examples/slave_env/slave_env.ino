...
#define DEVICE_BRAND "DHT Check App"
#define DEVICE_MODEL "DHT Environment"
...
const char sizeofFunctionList = 6;
...
#define WIRE_BEGIN 0x40

#define PIN_SENSOR_SMOKE A0
#define PIN_SENSOR_CARBONMONOXIDE A1
#define PIN_SENSOR_TEMPERATURE 13

SimpleDHT11 DHTSensor;
int lastStoredTemperature = 0;
int lastStoredHumidity = 0;
...
void setup() {
  ...
  // Initialize sensor device pin
  pinMode(PIN_SENSOR_SMOKE, INPUT);
  pinMode(PIN_SENSOR_CARBONMONOXIDE, INPUT);
...
void loop() {
  ...
  DHTSensor.read(PIN_SENSOR_TEMPERATURE, &temperature, &humidity, NULL);

  if (temperature != 0)
    lastStoredTemperature = (int)temperature;

  if (humidity != 0)
    lastStoredHumidity = (int)humidity;
}
...
void callReceivedFunction(unsigned short indexofFunction, unsigned short sizeofData, char data[]) {
  ...
  switch (indexofFunction) {
    ...
    case 1:
      ...
      break;
    case 2:
      getTemperature(sizeofData, data);
      break;
    case 3:
      getHumidity(sizeofData, data);
      break;
    case 4:
      getSmoke(sizeofData, data);
      break;
    case 5:
      getCarbonMonoxide(sizeofData, data);
      break;
    default:
      break;
  }
...
void getFunctionList(unsigned short sizeofData, char data[]) {
  ...
  char *result = Serialization.encode(11, delimiterBuffer, 12, resultBuffer);
  encodeData(strlen(result), result);
}

void getTemperature(unsigned short sizeofData, char data[]) {
  ...
  // Get temperature value and store it in given field
  char sensorResult[10];
  sprintf(sensorResult, "%d", (int)lastStoredTemperature);
...
void getHumidity(unsigned short sizeofData, char data[]) {
  ...
  // Get humidity value and store it in given field
  char sensorResult[10];
  sprintf(sensorResult, "%d", (int)lastStoredHumidity);

  char *delimiterBuffer = "";
  char *resultBuffer[] = {functionList[3],
                          sensorResult
                         };
  char *result = Serialization.encode(1, delimiterBuffer, 2, resultBuffer);
  encodeData(strlen(result), result);
...
void getSmoke(unsigned short sizeofData, char data[]) {
  ...
  // Get smoke value and store it in given field
  char sensorResult[10];
  sprintf(sensorResult, "%d", (int)analogRead(PIN_SENSOR_SMOKE));

  char *delimiterBuffer = "";
  char *resultBuffer[] = {functionList[4],
                          sensorResult
                         };
  char *result = Serialization.encode(1, delimiterBuffer, 2, resultBuffer);
  encodeData(strlen(result), result);
...
void getCarbonMonoxide(unsigned short sizeofData, char data[]) {
  ...
  // Get carbon monoxide value and store it in given field
  char sensorResult[10];
  sprintf(sensorResult, "%d", (int)analogRead(PIN_SENSOR_CARBONMONOXIDE));

  char *delimiterBuffer = "";
  char *resultBuffer[] = {functionList[5],
                          sensorResult
                         };
  char *result = Serialization.encode(1, delimiterBuffer, 2, resultBuffer);
  encodeData(strlen(result), result);
